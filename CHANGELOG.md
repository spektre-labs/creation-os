...
scaffolding with the σ-gate itself as the fitness function.  v3.0.0
proved σ-gate-per-turn; v3.1.0 turns the σ-gate into an optimisation
target that the system can apply *to its own tunable parameters*,
under a watchdog that refuses any mutation that does not strictly
drop Brier score on a labeled σ-fixture.

Scope discipline: this release ships the **scaffold**.  It is
evaluator-only (no LLM mutator); fitness runs on a hermetic synthetic
fixture; the entire loop is deterministic and self-tested.  Applying
it to live kernels on real workloads is the next experimental
campaign, not a shipped number in this release.  No overnight
miracle metrics are quoted anywhere — this is the infrastructure
that makes such a campaign possible and credible.

### New subsystem: Ω-stack (src/sigma/evolve/)

- **`cos_evolve`** — (1+1)-ES over the σ-ensemble weights +
  τ, using Brier on a labeled fixture as the fitness function.
  Convex in the weights for fixed per-row signals, so the
  self-test has a known-good attractor on every host.
- **`opt_memory`** — sqlite ledger of every mutation
  (accepted + reverted).  Schema includes an opaque `code_diff`
  column so LLM-mutator backends can later store C patches
  without migrations.  `memory-top` / `memory-stats` are indexed
  on `(kernel, accepted, delta)`.
- **`self_calibrate`** — τ-sweep on a 201-point grid with two
  loss modes: BALANCED `α·fa + (1-α)·fr` and RISK_BOUNDED
  `min fr s.t. fa ≤ budget`.  Maps cleanly onto the conformal
  operating point without re-running PAC calibration.
- **`discover`** — declarative hypothesis harness.  JSONL of
  (hypothesis, cmd, expect, expect_kind) → per-experiment
  verdict (`confirmed` / `rejected` / `inconclusive`) appended
  to `discoveries.jsonl`.  Not LLM-autogenerated; the scientist
  stays the human.

### New CLI surface: `cos-evolve`

Nine subcommands — `evolve · memory-top · memory-stats ·
calibrate-auto · discover · omega · daemon · demo · self-test`.
Persistent state under `$COS_EVOLVE_HOME` (default `~/.cos/evolve`):
`params.json`, `evolve_log.jsonl`, `memory.db`, `discoveries.jsonl`,
`omega_daemon.log`.

### Watchdog (OMEGA-6)

Foreground daemon loop with a σ-envelope rollback rule: three
consecutive iterations whose best Brier rises `> rise_limit` above
the rolling minimum trip an automatic rollback to the pre-iteration
snapshot and a clean halt with `reason=brier_rise`.  SIGINT / SIGTERM
halt cleanly with `reason=signal`.

### Check wiring

- `check-cos-evolve` harness exercises every subcommand on the
  20-row labeled fixture under `benchmarks/evolve/`.
- Folded into `check-sigma-pipeline`, so merge-gate runs the Ω
  scaffold on every final-commit validation.

### Documentation

- `docs/OMEGA_EVOLVE.md` — full Ω-stack reference with the
  Brier-convexity argument, schema specs, watchdog semantics,
  pluggable-mutator contract, and an explicit *"what this is not"*
  section consistent with `CLAIM_DISCIPLINE.md`.

### Version
- `COS_VERSION_STRING` → `3.1.0`
- `COS_CODENAME` → `OMEGA`
- `COS_CHECK_TARGETS` → 54 (+ check-cos-evolve)
