...
check-cos-moe:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import pytest" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run python -m pytest \
			...
			"$(CURDIR)/tests/test_omega.py" "$(CURDIR)/tests/test_omega_loop.py" "$(CURDIR)/tests/test_active_inference.py" "$(CURDIR)/tests/test_blanket.py" "$(CURDIR)/tests/test_autopoiesis.py" "$(CURDIR)/tests/test_phi_proxy.py" "$(CURDIR)/tests/test_channel.py" "$(CURDIR)/tests/test_dissipative.py" \
