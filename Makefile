...
check-cos-moe:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import pytest" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run python -m pytest \
			...
			"$(CURDIR)/tests/test_offline.py" "$(CURDIR)/tests/test_helm.py" "$(CURDIR)/tests/test_convergence.py" "$(CURDIR)/tests/test_steer.py" "$(CURDIR)/tests/test_steer_v2.py" "$(CURDIR)/tests/test_sae.py" "$(CURDIR)/tests/test_sae_v2.py" "$(CURDIR)/tests/test_scaling.py" "$(CURDIR)/tests/test_anti_hack.py" "$(CURDIR)/tests/test_autonomous.py" "$(CURDIR)/tests/test_strange_loop.py" "$(CURDIR)/tests/test_think_budget.py" "$(CURDIR)/tests/test_sigma_moe.py" "$(CURDIR)/tests/test_cost.py" \
