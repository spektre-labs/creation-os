...
check-cos-moe:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import pytest" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run python -m pytest \
			...
			"$(CURDIR)/tests/test_sigma_properties.py" "$(CURDIR)/tests/test_federated.py" "$(CURDIR)/tests/test_epistemic.py" "$(CURDIR)/tests/test_embodied.py" "$(CURDIR)/tests/test_moral.py" "$(CURDIR)/tests/test_energy.py" -q && \
