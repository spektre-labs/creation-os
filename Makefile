...
check-cos-moe:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import pytest" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run python -m pytest \
			...
			"$(CURDIR)/tests/test_gate.py" "$(CURDIR)/tests/test_probes.py" "$(CURDIR)/tests/test_discrimination.py" "$(CURDIR)/tests/test_hash_embedding.py" "$(CURDIR)/tests/test_health.py" "$(CURDIR)/tests/test_agi_demo.py" "$(CURDIR)/tests/test_edge.py" "$(CURDIR)/tests/test_criticality.py" -q && \
