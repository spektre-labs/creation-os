...
check-cos-moe:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import pytest" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run python -m pytest \
			...
			"$(CURDIR)/tests/test_reason.py" "$(CURDIR)/tests/test_reason_fol.py" "$(CURDIR)/tests/test_symbolic.py" "$(CURDIR)/tests/test_meta_goal.py" "$(CURDIR)/tests/test_drive.py" "$(CURDIR)/tests/test_v1_modules_deepening.py" \
			...
			"$(CURDIR)/tests/test_evolve.py" "$(CURDIR)/tests/test_evolve_v2.py" "$(CURDIR)/tests/test_mega.py" "$(CURDIR)/tests/test_boot.py" "$(CURDIR)/tests/test_engram.py" "$(CURDIR)/tests/test_bench.py" \
			...
			"$(CURDIR)/tests/test_offline.py" "$(CURDIR)/tests/test_helm.py" "$(CURDIR)/tests/test_convergence.py" "$(CURDIR)/tests/test_steer.py" "$(CURDIR)/tests/test_steer_v2.py" "$(CURDIR)/tests/test_sae.py" "$(CURDIR)/tests/test_sae_v2.py" "$(CURDIR)/tests/test_scaling.py" "$(CURDIR)/tests/test_anti_hack.py" "$(CURDIR)/tests/test_codegen.py" "$(CURDIR)/tests/test_architect.py" "$(CURDIR)/tests/test_autonomous.py" "$(CURDIR)/tests/test_strange_loop.py" "$(CURDIR)/tests/test_think_budget.py" "$(CURDIR)/tests/test_think_budget_v2.py" "$(CURDIR)/tests/test_sigma_moe.py" "$(CURDIR)/tests/test_cost.py" \
			...
			"$(CURDIR)/tests/test_sigma_properties.py" "$(CURDIR)/tests/test_federated.py" "$(CURDIR)/tests/test_epistemic.py" "$(CURDIR)/tests/test_embodied.py" "$(CURDIR)/tests/test_moral.py" "$(CURDIR)/tests/test_openended.py" "$(CURDIR)/tests/test_twin.py" "$(CURDIR)/tests/test_toolgen.py" "$(CURDIR)/tests/test_gemma_eval_lab.py" "$(CURDIR)/tests/test_run_all_smoke.py" "$(CURDIR)/tests/test_energy.py" -q && \
...
# Optional mutation testing (mutmut 3; Linux-friendly; slow). Install: `uv pip install mutmut`.
.PHONY: mutation
mutation:
	@if command -v uv >/dev/null 2>&1 && uv run python -c "import mutmut" >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" uv run mutmut run --max-children 4 && \
		uv run mutmut results && echo "mutation: OK"; \
	elif command -v mutmut >/dev/null 2>&1; then \
		PYTHONPATH="$(CURDIR)/python" mutmut run --max-children 4 && mutmut results && echo "mutation: OK"; \
	else \
		echo "mutation: SKIP (uv pip install mutmut)"; \
	fi
...
# Native C stack + v* subsystems (no PyPI); CycloneDX 1.5–style lite JSON → SBOM.json
sbom:
	@bash scripts/security/sbom.sh > SBOM.json
	@echo "sbom: OK (SBOM.json written in CycloneDX-lite 1.5 JSON form)"

# CycloneDX JSON for the active Python environment + pyproject (PyPI / SBOM).
# Base package declares ``dependencies = []``; extras are listed in pyproject.toml.
# Uses an ephemeral venv so ``pip install cyclonedx-bom`` works under PEP 668.
sbom-env:
	@cd "$(CURDIR)" && bash -euo pipefail -c '\
	  [ -f pyproject.toml ] || { echo "run from repo root" >&2; exit 1; }; \
	  VE="$$PWD/.venv-sbom-env"; \
	  rm -rf "$$VE"; \
	  python3 -m venv "$$VE"; \
	  "$$VE/bin/python" -m pip install -q -U pip cyclonedx-bom; \
	  "$$VE/bin/python" -m pip install -q -e .; \
	  "$$VE/bin/cyclonedx-py" environment --pyproject pyproject.toml --of JSON -o sbom.json "$$VE/bin/python"; \
	  rm -rf "$$VE"'
	@echo "SBOM generated: sbom.json (CycloneDX JSON)"
	@echo "Core [project] dependencies: 0 (see pyproject.toml)"
	@echo "Optional dependencies: pyproject.toml [project.optional-dependencies]"
