...
| Item | Change |
|------|--------|
| `cos_chat.c` | With **HTTP** + **`--multi-sigma`** (and **`--fast` off**): first completion at **T = 0.3** via `cos_bitnet_server_complete` (for **σ_logprob**); second and third at **T = 0.7** and **T = 1.2** via **`cos_bitnet_query_temp`** (malloc’d text, same JSON `temperature` path). Shadow **σ_consistency = 1 − mean pairwise `cos_text_jaccard`** over the three replies; **σ_combined = 0.7·σ_consistency + 0.3·σ_logprob** (T = 0.3 run). **`--fast`**: skip the triple. |
| `text_similarity.h/c` | **`cos_text_normalize`**, **`cos_text_jaccard`** (unique-word set Jaccard on normalized alnum tokens; cap **200** tokens per side). **`make check-text-similarity`** runs **`cos_text_similarity_self_check`**. |
