...
for name in reinforce speculative ttt engram moe multimodal tinyml; do
  [[ -s build/edge/"$name".o ]] || {
...
echo "check-edge-portability: PASS (7 σ primitives + tinyml demo build with -Os, no -march=native; tinyml binary ${tiny_sz} B)"
