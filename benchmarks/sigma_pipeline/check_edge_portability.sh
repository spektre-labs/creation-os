...
for name in reinforce speculative ttt engram moe multimodal tinyml swarm live continual unlearn agent diagnostic sovereign; do
  [[ -s build/edge/"$name".o ]] || {
...
echo "check-edge-portability: PASS (14 σ primitives + tinyml demo build with -Os, no -march=native; tinyml binary ${tiny_sz} B)"
