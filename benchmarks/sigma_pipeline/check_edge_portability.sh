...
for name in reinforce speculative ttt engram moe multimodal tinyml swarm; do
  [[ -s build/edge/"$name".o ]] || {
...
echo "check-edge-portability: PASS (8 σ primitives + tinyml demo build with -Os, no -march=native; tinyml binary ${tiny_sz} B)"
