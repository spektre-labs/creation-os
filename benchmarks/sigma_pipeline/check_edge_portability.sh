...
for name in reinforce speculative ttt engram moe multimodal tinyml swarm live; do
  [[ -s build/edge/"$name".o ]] || {
...
echo "check-edge-portability: PASS (9 σ primitives + tinyml demo build with -Os, no -march=native; tinyml binary ${tiny_sz} B)"
