...
| v103 σ-τ-Sweep secondary: σ_max_token (worst per-token σ along the continuation) outperforms entropy as a ranking signal | On `arc_challenge` Δ = −0.020 and `truthfulqa_mc2` Δ = −0.044 AURCC vs entropy; CIs overlap so the effect is suggestive but not significant at n=500.  Post-hoc, not pre-registered. | **P** (suggestive, needs n≥5 000 re-test) |
