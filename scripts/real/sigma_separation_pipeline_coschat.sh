...
cd "$ROOT"
PARSE_PY="$ROOT/scripts/real/sigma_coschat_parse.py"
...
; do
  ...
  printf '%s\n' "$OUT" >>"$LOG"
  read -r SIGMA ACTION ROUTE <<<"$(printf '%s' "$OUT" | python3 "$PARSE_PY")"
...
export PIPELINE_CSV="$CSV"
CHART="${OUTDIR}/chart.txt"
python3 - <<'PYCHART' | tee "$CHART"
import csv
import os
...
sigma = "\u03c3"
print("")
print(f"=== {sigma} Separation (cos chat pipeline) ===")
...
if vals:
    ...
    print("(no numeric sigma rows in CSV -- check cos output)")
PYCHART
