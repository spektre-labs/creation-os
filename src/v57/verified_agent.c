...
static const v57_slot_t k_slots[] = {
    ...
    {
        .slot           = "hyperscale_substrate",
        .owner_versions = "v70",
        .make_target    = "make check-v70",
        .best_tier      = V57_TIER_M,
        .summary        = "σ-Hyperscale: trillion-parameter / hyperscale-"
                          "killer substrate that turns a fleet (v69) into "
                          "a fleet-of-fleets on a single workstation — "
                          "ShiftAddLLM power-of-2 weight quantisation "
                          "(arXiv:2406.05981, NeurIPS 2024: weights as "
                          "±2^k, branchless multiply-free GEMV via "
                          "int32 shift + conditional negate, no DSP, no "
                          "matmul); Mamba-2 / Mamba-3 selective SSM scan "
                          "(arXiv:2312.00752 + arXiv:2603.15569: Q0.15 "
                          "recurrence, constant memory per token, "
                          "linear-time inference, no KV cache); RWKV-7 "
                          "\"Goose\" delta-rule step (arXiv:2503.14456: "
                          "O(1) per token, vector-valued gating + in-"
                          "context learning rate, exceeds Transformer "
                          "TC^0 under complexity conjectures); DeepSeek-"
                          "V3 auxiliary-loss-free MoE router scaled to "
                          "10 240 experts (arXiv:2412.19437: branchless "
                          "top-K MaxScore, integer bias controller "
                          "pushes underused experts up without an "
                          "auxiliary loss); Samsung HBM-PIM / RACAM "
                          "bit-serial AND-popcount surrogate "
                          "(arXiv:2603.09216: __builtin_popcountll over "
                          "column-major bit-packed weights; 9-102x "
                          "over-GPU on LLM inference); photonic "
                          "wavelength-multiplexed dot product "
                          "(SKYLIGHT arXiv:2602.19031 + LightMat-HP "
                          "arXiv:2604.12278 + Lightmatter Envise: 8 "
                          "parallel wavelength lanes as int32 "
                          "accumulators); Intel Loihi 3 graded-spike "
                          "sparse activation (January 2026, 8 M "
                          "neurons / 64 B synapses / 4 nm, 32-bit "
                          "graded spikes; MatMul-free LLM "
                          "arXiv:2503.18002 at 3.66 W on edge); NVIDIA "
                          "NCCL / Patarasuk-Yuan 2009 bandwidth-optimal "
                          "ring all-reduce (int32 saturating add over "
                          "2(N-1) steps, branchless topology-balance "
                          "check); Petals / Helix / DirectStorage / "
                          "GPUDirect Storage streaming weight scheduler "
                          "(64-slot LRU over (layer,expert) 64-bit key, "
                          "branchless monotone-clock victim, "
                          "__builtin_prefetch on insert); HSL — "
                          "Hyperscale Language — 10-opcode integer "
                          "bytecode ISA (HALT/SHIFT/SCAN/TIMEMIX/"
                          "ROUTEK/PIMPOP/WDM/SPIKE/RING/GATE) with "
                          "per-instruction silicon-unit cost accounting "
                          "and a GATE opcode that writes v70_ok iff "
                          "silicon cost ≤ budget AND throughput ≥ "
                          "floor AND topology balanced AND NOT "
                          "abstained; composes with v60..v69 as an "
                          "11-bit branchless AND "
                          "(cos_v70_compose_decision); ~187 M SSM "
                          "tokens/s N=4096, ~1.9 M RWKV-7 steps/s "
                          "dim=128, ~1.0 G AND-popcount ops/s "
                          "cols=4096, ~4 M HSL progs/s, ~187 M 11-bit "
                          "composed decisions/s; 148 034 deterministic "
                          "tests under ASAN+UBSAN (including full "
                          "2048-row 11-bit truth table)",
    },
};
